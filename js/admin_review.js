...
FCAPP.APPLY = {
    ...
    loadInfo: function() {
        ...
        var obj = document.getElementById("expert_photo"); //selectid
     	obj.value="图片大小70*70，小于50k";
    },
    ...
    uploadpicture:function(url){
    	...
    	data.url = url;
        data.cmd = 100;
        data.callback = 'uploadResult';
        window.uploadResult = APPLY.uploadResult;
		
        $.ajax({
            url: C.Upload + '?' + $.param(data),
            dataType: 'jsonp',
            type: 'POST'
    ...
    review_edit:function(){
        ...
        //var proimg = document.getElementById("imgpro"); 
    ...
    render_reivew_list:function(data){
    	...
    	var  $obj=$("#review_list");
    	var tpl = '<tr><td>{name}</td><td>{title}</td><td>{date}</td><td>待审核</td><td><a name="edit" class="blue" href="javascript:void(0)" id={ID}>编辑</a><a name="delete" class="blue" href="#none1" id={ID}>删除</a><a  target="_blank" class="blue" href="http://112.124.55.78/test/review.html?loupanid={loupanid}" >预览</a></tr>',
        List = [];
        for (var i = 0,
    ...
    review_edit_fill:function(id){
    	...
    	for(var i = 0,il = APPLY.g_ReviewList.length;i<il;i++){
    	 ...
    	 var obj = document.getElementById("proimg"); //selectid
    	 obj.src=review_info.photo;
